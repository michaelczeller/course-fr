...
# Data report: reminders, clarifications {background-color="#2c844c"}

- *intentionally* different from a 'normal' essay
    + less theory, literature review &rarr; more (basic) empirics
    + 'back to basics': *good* description before 'complex' arguments <!-- 'walking before running' &rarr; 'back to basics' -->
- (i) *description* is necessary for ii. *explanation* or iii. *prediction*
- *intentionally* vague requirements about data type
    + extant data, modified data, wholly new data---all possible
    + *in any case*, you should be thinking about how data is created and to what extent it is *[valid]{style="color:orange;"}* and *[reliable]{style="color:orange;"}*
- purpose: (1) data strength and [limitations]{style="color:yellow;"} awareness, (2) 'get hands dirty with data' (before thesis!), (3) building blocks for (thesis) research: [concepts]{style="color:orange;"}, [methods]{style="color:orange;"}, (good) [empirical material]{style="color:orange;"}

<!-- (i) *describe*, (ii) *explain*, (iii) *predict*---but *description* is basis for all -->
...
## drop rows with no Motto
de_demos_prep <- de_demos %>% drop_na(Motto)

de_corpus <- corpus(de_demos_prep$Motto)

docvars(de_corpus, "JAHR") <- (de_demos_prep$Jahr)
docvars(de_corpus, "LAND") <- (de_demos_prep$Land)
docvars(de_corpus, "ORT") <- (de_demos_prep$Ort)
docvars(de_corpus, "ZUOR") <- (de_demos_prep$Zuordnung)

de_corpus_stats <- summary(de_corpus, n = Inf)
de_corpus_stats <- as.data.frame(de_corpus_stats)

de_corpus_toks <- tokens(de_corpus, remove_punct=T, remove_numbers=T, remove_symbols=T)

de_corpus_dfm <- dfm(de_corpus_toks)

# library(stopwords)
de_corpus_dfm <- dfm_remove(de_corpus_dfm, 
                          c(stopwords("german"), 
                            "ist", "der", "others?"))

# de_corpus_dfm_trim <- dfm_trim(de_corpus_dfm, min_termfreq=2, max_termfreq=75)

de_corpus_dfm_stm <- convert(de_corpus_dfm, to="stm")

library(stm)
## https://juliasilge.com/blog/evaluating-stm/
# findingK <- searchK(de_corpus_dfm_stm$documents, de_corpus_dfm_stm$vocab,
#                     K=c(5:25), data=meta, verbose=FALSE)
# plot(findingK)
## held-out likelihood should be high; residuals should be low; lower bound should be low; semantic coherence should be at least somewhat low (without overspecificity) -- 15-16 K seems close to ideal

de_corpus_dfm_tm <- convert(de_corpus_dfm, to="topicmodels")

library(topicmodels)
de_corpus_tm_LDA <- LDA(de_corpus_dfm_tm, k=15)

library(tidytext)
de_corpus_tm_topics <- tidy(de_corpus_tm_LDA, matrix="beta")

de_corpus_tm_top_terms <- de_corpus_tm_topics %>% 
  group_by(topic) %>% 
  top_n(9, beta) %>% 
  ungroup() %>% 
  arrange(topic, -beta)

de_corpus_terms_plot <- de_corpus_tm_top_terms %>% 
  filter(topic %in% 1:9) %>% 
